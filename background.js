...
chrome.contextMenus.onClicked.addListener(function(){//項目をクリックされたら？
    ...
    chrome.tabs.query({active:true, currentWindow: true},function(tab){//今見てるウィンドウの今見てるタブの取得　permission:"tabs"が必要
        ...
        ページタイトル=ページタイトル.replace(/\|/g, "｜");// | がいるとどうも死ぬっぽいので置き換える。

        表示URL="https://x.com/intent/tweet?text=" + ページタイトル + "%0A&url=" + ページURL;
        //%0Aで改行

        chrome.tabs.query({url: "https://x.com/home"},function(tabs){//タブを全取得。　　　{currentWindow: true}でクティブウィンドウの中でのみにできる。わかりにくいならこれに切り替えるカモ
